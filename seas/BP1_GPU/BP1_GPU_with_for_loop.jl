...
tv = Array{Float64, 2}(undef, no_stored_time_steps, 2)
vv  = CuArray(convert(Array{Float64, 2}, zeros(no_stored_time_steps, length(stations)))) 
tv[1, 1] = t
tv[1, 2] = RSVinit
vv[1, 3:end] = δ_d[1:stride_space:δNp]
 ...
 odeparam = (
     ...
     prob = ODEProblem(odefun_GPU_with_for_loop, ψδ_d, tspan, odeparam)

     cb_mod = SavingCallback((ψδ, t, i) -> write_text_slip_GPU(ψδ, t, i, yf, stations, odeparam, "BP1_", 10 * year_seconds), SavedValues(Float64, Float64))
