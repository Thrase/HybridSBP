...
function get_operators(p, Nr, Ns, μ)
...
# Derivative operators for the rest of the computation
    (Dr, HrI, Hr, r) = diagonal_sbp_D1(p, Nr; xc = (0,1))
    Qr = Hr * Dr
    QrT = sparse(transpose(Qr))

    (Ds, HsI, Hs, s) = diagonal_sbp_D1(p, Ns; xc = (0,1))
    Qs = Hs * Ds
    ...
    Ir = sparse(I, Nrp, Nrp)
    Is = sparse(I, Nsp, Nsp)
    (_, S0r, SNr, _, _, Ar, _) = variable_diagonal_sbp_D2(p, Nr, μ * ones(Nrp); xc = (0,1))
    (_, S0s, SNs, _, _, As, _) = variable_diagonal_sbp_D2(p, Ns, μ * ones(Nsp); xc = (0,1))
    ...
    HfI_F1T = H1I * G1 - (τ1 ⊗ er0T)
    HfI_F2T = H2I * G2 - (τ2 ⊗ erNT)
    HfI_F3T = H3I * G3 - (es0T ⊗ τ3)
    HfI_F4T = H4I * G4 - (esNT ⊗ τ4)
...
function bdry_vec_mod!(g, F, τ, r, s, bc_Dirichlet, bc_Neumann)
    ...
    # FACE 1 (Dirichlet):
    vf = bc_Dirichlet(1, zeros(size(r)), s)
    g[:] -= F[1] * vf

    # FACE 2 (Dirichlet):
    vf = bc_Dirichlet(2, ones(size(r)), s)
    g[:] -= F[2] * vf
...
function bdry_vec_mod_GPU!(g, F, δ, Vp, t, cuarry_ones)
...
function computetraction_stripped(HfI_FT, τ, lf, u, δ)
    ...
    return (HfI_FT * u + τf * (δ - 0.5 * δ)) 
  end
  
  function computetraction_stripped_GPU!(Δτ, HfI_FT, τ, lf, u, δ)
    HfI_FT = HfI_FT[lf]
    τf = τ[lf]

    Δτ[:] = (HfI_FT * u + τf * (δ - 0.5 * δ)) 
    return nothing
  end
